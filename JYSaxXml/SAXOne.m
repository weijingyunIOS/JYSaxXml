 ...
 */
+ (void)saxparserWithdata:(NSData *)data videosDic : (NSDictionary*)videosDic modelClass : (Class) modelClass finished:(void (^)(NSArray *))finished  {
...
didStartElement:(NSString *)elementName
 ...
 qualifiedName:(NSString *)qName
    ...
    //    <video videoId="1">
    // 判断节点 是否是 video 如果是就建立  数据模型
    if ([elementName isEqualToString:self.videosDic[JYNodeDictionary]]) {
        
        self.currentVideo = [[self.modelClass alloc] init];
...
- (void)parser:(NSXMLParser *)parser
 ...
 qualifiedName:(NSString *)qName {
    ...
    //  节点video将模型添加到数组  videos节点表示完成了，其它节点作为模型的key传入
    if ([elementName isEqualToString:self.videosDic[JYNodeDictionary]]) {
        [self.videos addObject:self.currentVideo];
    } else if (![elementName isEqualToString:self.videosDic[JYNodeArray]]){
        // 使用KVC给模型传值
